...
**`/dashboard` (Next.js)** -- The operator-facing web UI. Built with Next.js 16, React 19, and D3 charting. Username/password login creates a signed HttpOnly dashboard session; same-origin server routes attach the API token without exposing it to browser JavaScript, while WebSockets use short-lived signed tickets. The UI provides live engagement monitoring, attacker profiles, kill-chain visualization, ATT&CK coverage heatmaps, canary status, template management, service health, Theater queue/action workflows (`/theater`), and replay drilldown workflows (`/theater/replay/{session_id}`). Dashboard websocket clients include reconnect/backoff handling and freshness telemetry indicators for degraded stream visibility.
